...
def get_tweets():
    ...
    tweetFile = open('2018_dartmouth_tweets.csv', encoding="utf-8")
...
if __name__ == '__main__':
    ...
    def getDataSize(year, months, days):
        dataSize = 0
        for month in months:
            for day in days:
                if(int(day) <= assocs[month]):
                    dataSize += 1
        return dataSize
    x = np.arange(1,getDataSize(year, monthNums, dayNums) + 1)
    y = []
